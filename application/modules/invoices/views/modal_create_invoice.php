...
function dateCheck(){
// 	var today = new Date();
	var mydate = $('#invoice_date_createds').val();
// 	var md = new Date(mydate);
// 	var status="";

// 	var date1=today.getTime();
// 	var date2=md.getTime();
	
// 	var nowyear=today.getUTCFullYear();
// 	var selyear=md.getUTCFullYear();
	
// 	nowyear>selyear

// 	if(date1>date2){
// 	status="you cannot enter previous date.";
// 	alert(status);
// 	$('#invoice_create_confirm').attr('disabled',true);
// 	}else if(date1<=date2){
// 	$('#invoice_create_confirm').attr('disabled',false);
// 	}
	

	$.post("<?php echo site_url('invoices/ajax/setsessiondate'); ?>", {
        cdate: mydate
    },
    function (data) {
//         alert(data);
...
function myCheck(){
    var clid = $('#create_invoice_client_id').val();
    var dat = $('#invoice_date_createds').val();
    if(clid==0 && dat==""){
        $('#invoice_create_confirm').attr('disabled',true);
    }else if(clid!=0 && dat!=""){
        $('#invoice_create_confirm').attr('disabled',false);
...
function mc(){
    ...
    $(function () {
        ...
        $('#invoice_date_createds').val(month+"/"+date+"/"+year);
        ...
        // $("#invoice_date_created").val(cdate);
        ...
        $('#invoice_create_confirm').click(function () {
            ...
            $.post("<?php echo site_url('invoices/ajax/create'); ?>", {
                    ...
                    invoice_date_created: $('#invoice_date_createds').val(),
...
<div id="create-invoice" class="modal modal-lg" role="dialog"
	aria-labelledby="modal_create_invoice" aria-hidden="true">
	<form class="modal-content">
		...
		<div class="modal-body">
			...
			<!--                   value="-->
			<?php //echo get_setting('enable_permissive_search_clients'); ?>
			<!--">-->
			...
			<div class="form-group has-feedback">
				...
				<div class="input-group">
					...
					<!--                             autofocus="autofocus"> -->
                        ...
                        <?php //endif; ?>
<!--                     </select> -->
					<select onchange="myCheck()"  name="client_id" id="create_invoice_client_id"
						class="form-control simple-select">
						<option value="0"><?php _trans('select_client'); ?></option>
                                <?php foreach ($clients as $client) : ?>
                                    <option
							value="<?php echo $client->client_id; ?>"
							<?php check_select($this->mdl_clients->client_lookup('client_name'),  $client->client_name); ?>><?php echo $client->client_name; ?></option>
                                <?php endforeach; ?>
                            </select> <span
						id="toggle_permissive_search_clients" class="input-group-addon"
						title="<?php _trans('enable_permissive_search_clients'); ?>"
						style="cursor: pointer;"> <i
						class="fa fa-toggle-<?php echo get_setting('enable_permissive_search_clients') ? 'on' : 'off' ?> fa-fw"></i>
					</span>
				</div>
			</div>
			...
			<div class="form-group has-feedback">
				...
				<div class="input-group">
					<input  autocomplete="off"   name="invoice_date_createds" id="invoice_date_createds"
						class="form-control datepicker"
						 value=""
						required="required" onkeyup="mc()" onchange="dateCheck()"> <span
			...
			<div class="form-group">
				...
				<input type="text" name="invoice_password" id="invoice_password"
					class="form-control"
			...
			<div class="form-group">
				...
				<select name="invoice_group_id" id="invoice_group_id"
                    ...
                    <?php foreach ($invoice_groups as $invoice_group) { ?>
                        <option
						value="<?php echo $invoice_group->invoice_group_id; ?>"
		...
		<div class="modal-footer">
			<div class="btn-group">
				<button class="btn btn-success ajax-loader"
					id="invoice_create_confirm" type="button">
