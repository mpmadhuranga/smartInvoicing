...
class Ajax extends Admin_Controller
{
    ...
    public function filter_suppliers()
    {
        ...
        foreach ($keywords as $keyword) {
            if ($keyword) {
                $keyword = strtolower($keyword);
                $this->mdl_suppliers->like("CONCAT_WS('^',LOWER(supplier_name),LOWER(supplier_contact_name),LOWER(supplier_email),supplier_phone,supplier_active)", $keyword);
            }
        }

        $data = array(
            'records' => $this->mdl_suppliers->with_total_balance()->get()->result()
        );

        $this->layout->load_view('suppliers/partial_supplier_table', $data);
    }
    ...
    public function filter_products()
    {
        ...
        foreach ($keywords as $keyword) {
            if ($keyword) {
                $keyword = strtolower($keyword);
                $this->mdl_products->like("CONCAT_WS('^',LOWER(product_sku),LOWER(product_name),product_price,purchase_price,product_qty,LOWER(provider_name))", $keyword);
            }
        }
