...
class Mdl_Payments extends Response_Model
{
    ...
    public function default_join()
    {
        ...
        $this->db->join('ip_users', 'ip_payments.user_id = ip_users.user_id');
    ...
    public function validation_rules()
    {
        return array(
            ...
            'received_manager' => array(
                'field' => 'received_manager',
                'label' => trans('received_manager')
            ),
            'received_admin' => array(
                'field' => 'received_admin',
                'label' => trans('received_admin')
            )
        );
    }
    ...
    public function save($id = null, $db_array = null)
    {
        ...
        if ($paid >= $total) {
            ...
            $this->db->where('invoice_id', $db_array['invoice_id']);
            $this->db->set('invoice_status_id', 6);
            $this->db->update('ip_invoices');
        }
    ...
    public function db_array()
    {
        ...
        $db_array['user_id'] = $this->session->userdata('user');

        return $db_array;
    }


    public function update_image_path($image_name, $id){
        $data = array(
            'file_name_original' => $image_name
    ...
    public function insert_file($id,$filename, $title)
    {
        $data = array(
            'filename'      => $filename
        );
