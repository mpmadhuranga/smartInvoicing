...
class Payments extends Admin_Controller
{
    ...
    public function form($id = null)
    {
            ...
            if (!empty($_FILES['imagefile']['name'])){
                ...
                if(in_array($extension,$allowed_extension)){
                    ...
                    if(move_uploaded_file($_FILES['imagefile']['tmp_name'],$path)){
                        ...
                        $this->mdl_payments->update_image_path($imageName, $id);
                    }
                }
            }
        ...
        //  $this->mdl_payments->update_image_path($image_name, $id);
    ...
    public function image_upload()
    {
        if (isset($_FILES["imagefile"]['name'])){
            $data=explode('.',$_FILES['imagefile']['name']);
            $extension=$data[1];
            $allowed_extension=array("jpg","png","jpg");
            if(in_array($extension,$allowed_extension)){
                $new_file_name=rand().'.'.$extension;
                $path='uploads/'.$new_file_name;
                if(move_uploaded_file($_FILES['imagefile']['tmp_name'],$path)){
    ...
    public function savecheque()
    {
        ...
        $data = array(
            'supplier_id' => 1,
            'added_date' => date_to_mysql($addeddate),
            'amount' => $amounts,
            'check_no' => $checkno,
            'clear' => 1
        );

        $this->db->insert('ip_cheques', $data);
    }
